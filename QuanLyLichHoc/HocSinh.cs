        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            VALUES (@MaHS, @HoTen, @GioiTinh, @NoiSinh, @TrangThai, @Nghi, @Email, @Lop)";
                    ...
                    using (SqlCommand insertUserCmd = new SqlCommand(insertUserQuery, conn))
                    {
                        ...
                        insertUserCmd.Parameters.AddWithValue("@MK", "123"); // Mật khẩu mặc định
                        insertUserCmd.Parameters.AddWithValue("@VaiTro", "Student"); // Vai trò mặc định là học sinh
        ...
        private void btnEdit_Click(object sender, EventArgs e)
{
    if (dgvHocSinh.SelectedRows.Count > 0)
    {
        ...
        {
            MessageBox.Show("Vui lòng điền đầy đủ thông tin vào tất cả các ô!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return; // Dừng thực thi nếu có ô trống
        }
        ...
        try
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                // Kiểm tra lớp học có tồn tại trong bảng LOPHOC hay chưa
                string checkClassQuery = "SELECT COUNT(*) FROM LOPHOC WHERE MaLop = @MaLop";
                using (SqlCommand checkClassCmd = new SqlCommand(checkClassQuery, conn))
                {
                    ...
                    if (classExists == 0)
                    {
                        // Thêm lớp mới vào bảng LOPHOC nếu chưa tồn tại
                        string insertClassQuery = @"
                        INSERT INTO LOPHOC (MaLop, TenLop, PhongHoc) 
                        VALUES (@MaLop, @TenLop, @PhongHoc)";
                        using (SqlCommand insertClassCmd = new SqlCommand(insertClassQuery, conn))
    ...
    else
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dgvHocSinh.SelectedRows.Count > 0)
            {
                ...
                string maHS = dgvHocSinh.SelectedRows[0].Cells["MaHS"].Value.ToString();
                string email = dgvHocSinh.SelectedRows[0].Cells["Email"].Value.ToString(); // Lấy Email của học sinh
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(connectionString))
                    {
                        ...
                        // Xóa dữ liệu liên quan đến học sinh trong các bảng khác (e.g., DIEM, DIEMTAP)
                        ...
                        // Sau khi xóa học sinh, xóa tài khoản của học sinh trong bảng USERS
        ...
        {
            // Giả sử txtMaHS là TextBox chứa mã học sinh
            string maHS = txtMaHS.Text; // Lấy mã học sinh từ form HocSinh
            ...
            // Mở form QuanLyBT và truyền MaHS vào constructor
            QuanLyBT quanLyBTForm = new QuanLyBT(userSession); // Giả sử vai trò là "Student"
            quanLyBTForm.Show();
        }
